                                   ...
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ...
    // TEMP FIX: color_space removed in ESP-IDF v6.x, use rgb_ele_order instead
    // Use rgb_ele_order from panel_dev_config (available in ESP-IDF v6.x)
    if (panel_dev_config->rgb_ele_order == LCD_RGB_ELEMENT_ORDER_RGB) {
        st7703->madctl_val = 0;
    } else if (panel_dev_config->rgb_ele_order == LCD_RGB_ELEMENT_ORDER_BGR) {
