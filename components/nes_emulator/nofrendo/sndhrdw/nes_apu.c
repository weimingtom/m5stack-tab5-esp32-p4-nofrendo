...
{
   // Логирование записей в APU регистры для диагностики (первые 50 записей)
   static int write_count = 0;
   if (write_count++ < 50) {
       ESP_LOGI(APU_TAG, "apu_write #%d: %04X = %02X", write_count, address, value);
   }
...
void apu_setparams(double base_freq, int sample_rate, int refresh_rate, int sample_bits)
{
   ...
   ESP_LOGI(APU_TAG, "apu_setparams: sample_rate=%d, refresh_rate=%d, num_samples=%d, sample_bits=%d", 
            sample_rate, refresh_rate, apu.num_samples, sample_bits);
   if (0 == base_freq)
